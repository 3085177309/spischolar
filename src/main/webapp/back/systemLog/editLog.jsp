		...
		<ul class="side-nav">
			<c:forEach items="${permission.xtrz }" var="xtrz" varStatus="status">
				<li><a href="<cms:getProjectBasePath/>${xtrz.url}"
					<c:if test="${xtrz.id == 28 and log.type ==1}">class="in"</c:if>
					<c:if test="${xtrz.id == 30 and log.type ==0}">class="in"</c:if>>${xtrz.columnName }</a></li>
			</c:forEach>
		</ul>
						...
						<table class="article-edit">
							<tr>
								...
								<td><span><input type="text" name="title"
										value="${log.title }" class="input-text"></span></td>
							</tr>
							<tr>
								...
								<td><span class="log">
										<div class="datebox">
											<i class="inc uv13"></i> <input type="text" name="releases"
												value="${log.releases.substring(0,10) }"
												class="input-text attr-input"
							...
							<tr>
								...
								<td class="pdtextarea"><span> <input
										name="logAbstract" type="hidden" value="" />
							...
							<tr>
								<th><span>附件：</span></th>
								<td><c:if test="${method == 'updateLog' }">${log.name }</c:if>
									<span class="uploadfile modify"><i></i>上传</span> <input
								...
								<td>
									<div>
										<input type="hidden" name="type" id="type"> <input
											type="hidden" name="lable" id="lable" value="">
										<p class="tagsShow">

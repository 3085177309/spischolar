...
<div id="content">
	<div class="col-left left-menue" id="side-menue">
		<h3>
			<span class="inc uv12"></span> <span class="inc uv40"></span> 内容分析
		</h3>
		<ul class="side-nav">
 ...
 -->
			...
			<div class="scroll">
				<div class="pageTopbar clearfix">
					<form id="comefrom" action="" method="get">
						<input type="hidden" name="school" id="school"> <label
				...
				<div class="radius">
					<table class="data-table">
						<tr>
							<th>序号</th>
							<th>学校</th>
							<th class="sort count" type="count" sort='down'>总下载次数</th>
							<th class="sort titleCount" type="titleCount" sort='down'>下载文章数</th>
							<th class="sort avg" type="avg" sort='down'>篇均下载次数</th>
							<th class="sort avgPageByDay" type="avgPageByDay" sort='down'>日均下载次数</th>
						</tr>
						<c:forEach items="${pager.rows }" var="list" varStatus="status">
							<tr>
								<td>${status.index+1+ offset }</td>
								<td>${list.school }</td>
								<td><a class="schoolSearch" org_falg="${list.schoolFlag }">${list.count }</a></td>
								<td>${list.titleCount }</td>
								<td>${list.avg }</td>
								<td>${list.avgPageByDay }</td>
							</tr>
						</c:forEach>
					</table>
				</div>
	...
	<div class="tickbox">
		<div id="export" class="export" data-tit="导出">
			<form id="exportForm"
				action="<cms:getProjectBasePath/>backend/contentAnalysis/downloadRecord/download"
				method="get">
				<ul>
					...
					<li><label for="" class="data-type"> <span
							...
							<div class="datebox">
								<i class="inc uv13"></i> <input type="text"
									...
									id="exportBeginTime" value="${beginTime }" readonly="readonly"
									name="beginTime"
					...
					<li><label class="data-type"> <span class="labt">数据类型:</span>
							<div class="sc_selbox">
								...
								<div class="sc_selopt" style="display: none;">
									<p onclick="change(-1,2)">所有数据</p>
									<p onclick="change(0,2)">原始数据</p>
									<p onclick="change(1,2)">添加数据</p>
								</div>
								<input type="hidden" id="downType" name="type" value="${type }">
							</div>
					</label></li>

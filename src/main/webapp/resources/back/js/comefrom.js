...
where[34] = new comefrom("其它","|北美洲|南美洲|亚洲|非洲|欧洲|大洋洲");
	...
	function select(ele) {
		...
		var loca2=ele.innerHTML;
		var city=document.getElementById("city");
		var cityOption=city.getElementsByTagName("div")[0];
	//	city.getElementsByTagName("span")[0].innerHTML="请选择城市";
		cityOption.innerHTML="";	
		for(var i = 0;i < where.length;i ++) {
			if (where[i].loca == loca2) {
				loca3 = (where[i].locacity).split("|");
				for(j = 1;j < loca3.length;j++) { 
					cityOption.innerHTML=cityOption.innerHTML+"<p>"+loca3[j]+"</p>"
				}
				break;
			}
		}
		...
		var cityOpts=city.getElementsByTagName("p");
		for (var i = 0; i < cityOpts.length; i++) {
			cityOpts[i].onclick=function(event){
				if (window.event) {
	            	window.event.cancelBubble=true;// ie下阻止冒泡
	            }else {
	            	event.stopPropagation();
	            } 
				city.getElementsByTagName("span")[0].innerHTML=this.innerHTML;
	...
	function init() {
		...
		var provinceOption=province.getElementsByTagName("div")[0];
		for(var k=0;k<where.length;k++){
			provinceOption.innerHTML=provinceOption.innerHTML+'<p>'+where[k].loca+"</p>"
		}
		
		province.onclick=function(event){
			provinceOption.style.display="block";
		}
		province.onmouseout=function(event){
			var eventobj=event||window.event;
			if(isMouseLeaveOrEnter(eventobj,this,'div')){
				this.getElementsByTagName("div")[0].style.display="none";
			}
		}
		var provinceOpts=provinceOption.getElementsByTagName("p");
		for (var i = 0; i < provinceOpts.length; i++) {
			provinceOpts[i].onclick=function(event){
				...
				province.getElementsByTagName("span")[0].innerHTML=this.innerHTML;
				select(this);
			}
		};
		var defaultProvince=province.getElementsByTagName("span")[0];
		if(defaultProvince.innerHTML!="请选择省份"){
			select(defaultProvince);
		}

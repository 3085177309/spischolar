...
function Scroll(el, options) {
	...
	this.options = {
	    ...
	    bindToWrapper : false,		// 事件是否绑定到 wrapper 元素上，否则大部分绑定到 window（若存在嵌套，则绑定在元素上最好）
	...
	// ==================================
		...
		this.options.momentum = false;
		/* modified new Add Lee*/
		this.options.freeScroll = true;
		/* modified Add Lee*/
		this.scroller = $('.ui-tab-content',this.wrapper)[0];
						//$(this.wrapper).find('.ui-slider-content')[0];
		this.nav = $('.ui-tab-nav',this.wrapper)[0];

		$(this.scroller.children[this.options.currentPage]).addClass('current');
		$(this.nav.children[this.options.currentPage]).addClass('current');
		...
		this.itemWidth = this.scroller.children[this.options.currentPage].clientWidth;
		this.scrollWidth = this.itemWidth * this.count;
	...
	this.translateZ = utils.hasPerspective && this.options.HWCompositing ? ' translateZ(-20px)' : '';
	...
	if(this.options.role === 'tab'){
		...
		this.options.directionLockThreshold=1;
		this.options.scrollY=true;

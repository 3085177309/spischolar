...
public class ZtfxServiceImpl implements ZtfxServiceI {
    ...
    @Override
    public String getZtpc(String jguid, int startYear, int endYear) {
    ...
    @Override
    public String getFwqs(String jguid, int startYear, int endYear) {
        JSONObject pageData = new JSONObject();
    ...
    @Override
    public String getTfzt(String jguid, int startYear, int endYear) {
        JSONObject pageData = new JSONObject();
        //发送到前台的对象
        ...
        if (getResp.isExists()) {
            ...
            //对突发主题进行排序
            Iterator<String> tfztYearData = pageData.keys();
    ...
    public String getFwqk(String keyword) {
        ...
        for (int i = 0; i < size; i++) {
            lenged.add(fwqk.getJSONObject(i).getString("journal") + "[" + fwqk.getJSONObject(i).getString("jguid") + "]");
    ...
    public String getMoreFwqsForKey(String keyword, int startYear, int endYear) {
        ...
        for (org.elasticsearch.search.aggregations.bucket.histogram.Histogram.Bucket bucket : years.getBuckets()) {
    ...
    private SearchResponse queryFwqsForKey(String keyword, int startYear, int endYear) {
        ...
        SearchResponse resp = client.prepareSearch(Comm.WOS_INDEX).setTypes("periodical")
                .setQuery(queryBuilder)
                .setSize(0)
                .addAggregation(year)
                .get();
    ...
    private void sortJsonArray(JSONArray arr) {
        Collections.sort(arr, new Comparator<JSONObject>() {
            ...
            public int compare(JSONObject o1, JSONObject o2) {
                ...
                double b = o2.getDouble("value");//Double.parseDouble(o2.values().toArray()[0].toString());
                if (a == b) {
                    return 0;
                } else if (a > b) {
                    return -1;

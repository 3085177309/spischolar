...
public class SimpleMailSender {
	 ...
	 *            待发送的邮件的信息
	 */
	public boolean sendTextMail(MailSenderInfo mailInfo) throws SystemException{
		// 判断是否需要身份认证
		...
		try {
			...
			throw new SystemException("文件发送出错!",ex);
		}
	}
	...
	public static boolean sendHtmlMail(MailSenderInfo mailInfo) {
		...
		try {
			...
			List<File> attachs=mailInfo.getAttcheFiles();
			if(attachs!=null){
				for(File attach:attachs){
					String affixName=attach.getName();
			...
			//log.error("邮件发送至"+mailInfo.getToAddress()+"失败!", ex);
			ex.printStackTrace();
			return false;
			//throw new RuntimeException("邮件发送"+mailInfo.getToAddress()+"失败!",ex);
		}
	}
}

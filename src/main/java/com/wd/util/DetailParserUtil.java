...
import com.wd.front.bo.ShouluDetail;
...
public class DetailParserUtil {
	...
	public static Map<String,ShouluMap> parseShoulu(List<Map<String, Object>> shoulus) throws SystemException{
		...
		final List<AuthorityDatabase> dbs=cacheModule.findDbPartitionFromCache();
	...
	public static Map<String,Object> parseShoulu() throws SystemException{
		CacheModuleI cacheModule = SpringContextUtil.getBean(CacheModuleI.class);
		final List<AuthorityDatabase> dbs=cacheModule.findDbPartitionFromCache();
		...
		for (int i = 0; i < dbs.size(); i++) {
			result.put(dbs.get(i).getFlag(), dbs.get(i));
		}
